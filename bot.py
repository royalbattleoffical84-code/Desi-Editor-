...
def handle_start(message):
    ...
    if len(args) > 1 and user_id != OWNER_ID:
        ...
        if ref_id != str_user_id and str_user_id not in referrals.get('tracked', []):
            ...
            referrals[ref_id]['bonus_likes'] += 1  # Referral se +1 extra like limit bonus milega
            save_data()
            try:
    ...
    bot.reply_to(
        message,
        "✅ *Welcome to SXO LIKE BOT✨!*\n\n"
        "Neeche menu button se apna zaroori option select karo, ya `/like {region} {uid}` command use karo.",
        parse_mode='Markdown',
...
def handle_verify(call):
    ...
    if is_user_member(user_id):
        ...
        bot.edit_message_text(
            "🎉 *Verification Successful!*\n\nAb aap neeche diye menu ka use kar sakte ho.",
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            parse_mode='Markdown'
        )
        bot.send_message(call.message.chat.id, "👇 Menu se option choose karo:", reply_markup=get_main_menu_keyboard())
    else:
        bot.answer_callback_query(call.id, "❌ Aap abhi tak channel join nahi kiye hain!", show_alert=True)
...
def handle_menu_buttons(message):
    ...
    if text == "👤 My Profile":
        ...
        if daily_bonus.get(str_user_id) == today:
            bot.reply_to(message, "❌ Aap aaj ka daily bonus pehle hi le chuke hain! Kal phir try karo.", parse_mode='Markdown')
        else:
        ...
        ref_msg = (
            f"👥 *REFERRAL SYSTEM*\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"Har referral pe milega bonus like limit!\n\n"
            f"🔗 *Your Ref Link:*\n`{ref_link}`\n\n"
        ...
        # Top 5 referral list banate hain
        ...
        for uid, data in sorted_refs:
            if uid != 'tracked' and isinstance(data, dict):
                ...
                rank += 1
        if rank == 1:
            lb_text += "Abhi tak koi bhi leaderboard me nahi hai!"

        bot.reply_to(message, lb_text, parse_mode='Markdown')

    elif text == "⚡ Send Like":
        bot.reply_to(message, "🎮 Like bhejne ka sahi tarika:\n\n`/like {region} {uid}`\n\nExample:\n`/like bd 10832316022`", parse_mode='Markdown')

    elif text == "🛠 Support":
        markup = telebot.types.InlineKeyboardMarkup()
        markup.add(telebot.types.InlineKeyboardButton("💬 Join Support Group", url="https://t.me/Avg_Official_1"))
        bot.reply_to(message, "🛠 Koi bhi problem ho to hamare support group me contact karo:", reply_markup=markup)
...
if __name__ == "__main__":
    print("🚀 SXO LIKE BOT✨ Iꜱ Rᴜɴɴɪɴɢ 🏃‍♂️ (All Requested Features Added)")
    while True:
        try:
